...
class Addchu_model extends CI_Model
{
    public $table = 'addchu';
    public $id;
    public $uid;
...
//
    ...
    public function get_list()
    {
		...
		$this->db->where('addchu.is_del', "0");//0没有删除
		$this->db->select('addchu.id as id,addchu.name as name,addchu.price as price,addchu.uid as uid,
		addchu.addtime as addtime,addchu.volume as volume,addchu.area as area,wxuser.is_use as is_use,wxuser.nickName as username');
		$this->db->order_by('addchu.addtime','desc');
		$this->db->from($this->table);
		$this->db->join('wxuser', 'addchu.uid = wxuser.id');
		$query = $this->db->get();
