...
class Addbuy_model extends CI_Model
{
    public $table = 'addbuy';
    public $id;
    public $uid;
...
//
    ...
    public function get_list()
    {
        ...
        $this->db->where('addbuy.is_del', "0");//0没有删除
		$this->db->order_by('addbuy.addtime','desc');

		$this->db->select('addbuy.id as id,addbuy.name as name,addbuy.price as price,addbuy.uid as uid,
		addbuy.addtime as addtime,addbuy.volume as volume,addbuy.area as area,wxuser.is_use as is_use,wxuser.nickName as username');



		$this->db->from($this->table);
		$this->db->join('wxuser', 'addbuy.uid = wxuser.id');
