...
class Addsell_model extends CI_Model
{
    public $table = 'addsell';
    public $id;
    public $uid;
    public $name;
    public $year;
...
//
    ...
    public function get_list()
    {
		...
		$this->db->where('addsell.is_del', "0");//0没有删除
		$this->db->order_by('addsell.addtime','desc');
		$this->db->select('addsell.id as id,addsell.name as name,addsell.price as price,addsell.uid as uid,
		addsell.addtime as addtime,addsell.volume as volume,addsell.area as area,wxuser.is_use as is_use,wxuser.nickName as username');

		$this->db->from($this->table);
		$this->db->join('wxuser', 'addsell.uid = wxuser.id');
		$query = $this->db->get();
